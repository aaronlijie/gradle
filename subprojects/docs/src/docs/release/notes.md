...
The `Delete` task automatically uses the `@Destroys` annotation, so any files added via its `delete()` API will be safe when running in parallel.  For instance, it should be safe to run `clean build` or `build clean` while using `--parallel` now.
